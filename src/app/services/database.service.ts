...
async readUser(userName: string): Promise<User | undefined> {
  try {
    ...
    const result = await this.db.query(q, [userName]);

    if (result.values && result.values.length > 0) {
      // Convierte el resultado de la consulta a un objeto User
      return this.rowToUser(result.values[0]);
    } else {
      // Si no se encuentra el usuario, devuelve undefined
      return undefined;
    }
  } catch (error) {
